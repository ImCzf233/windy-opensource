    ...
    protected void processTask(final ChunkCompileTaskGenerator generator) throws InterruptedException
    {
        ...
        try
        {
            if (generator.getStatus() != ChunkCompileTaskGenerator.Status.PENDING)
            {
                ...
                {
                    LOGGER.warn("Chunk vision task was " + generator.getStatus() + " when I expected it to be pending; ignoring task");
                }
        ...
        else
        {
            ...
            try
            {
                if (generator.getStatus() != ChunkCompileTaskGenerator.Status.COMPILING)
                {
                    if (!generator.isFinished())
                    {
                        LOGGER.warn("Chunk vision task was " + generator.getStatus() + " when I expected it to be compiling; aborting task");
                    }
                ...
                public void onSuccess(List<Object> p_onSuccess_1_)
                {
                    ...
                    label21:
                    {
                        try
                        {
                            ...
                            if (!generator.isFinished())
                            {
                                ChunkRenderWorker.LOGGER.warn("Chunk vision task was " + generator.getStatus() + " when I expected it to be uploading; aborting task");
                            }
                        }
                        finally
                        {
