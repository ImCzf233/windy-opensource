...
import space.emptiness.events.rendering.EventDrawText;
...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        if (this.showCrosshair() && this.mc.gameSettings.thirdPersonView < 1)
        {
            ...
            if(!(HUD.crosshair.getValue() && ModuleManager.getModuleByName("HUD").isEnabled())) {
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks)
    {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer)
        {
            ...
            float f = this.zLevel;
            this.zLevel = -90.0F;
            int x = i - 91 - 1 + entityplayer.inventory.currentItem * 20 + 1;
            this.anim = (float) RenderUtil.getAnimationState(this.anim,x,Math.max(50.0f, Math.abs(this.anim - (float)x) * 10.0f));
            if(HUD.blackhotbar.getValue() && ModuleManager.getModuleByClass(HUD.class).isEnabled()) {

                Gui.drawRect(i - 91, sr.getScaledHeight() - 23, i + 91, sr.getScaledHeight() - 22, ColorUtils.fastrainbow().getRGB());
                Gui.drawRect(i - 91, sr.getScaledHeight() - 22, i + 91, sr.getScaledHeight(), new Color(0, 0, 0, 120).getRGB());
                Gui.drawRect(this.anim, sr.getScaledHeight() - 22, this.anim + 22, sr.getScaledHeight(), new Color(255,255,255,50).hashCode());
            } else {
    ...
    {
        space.emptiness.module.modules.visual.Scoreboard scoreboard23 = (space.emptiness.module.modules.visual.Scoreboard) Client.instance.getModuleManager().getModuleByClass(space.emptiness.module.modules.visual.Scoreboard.class);
        if (scoreboard23.NoScore.getValue() == true && scoreboard23.isEnabled()) {
            return;
        }
        ...
        int k1 = p_180475_2_.getScaledHeight() / 2 + j1 / 3 + scoreboard23.Y.getValue().intValue();
        byte b0 = 3;
        int j = p_180475_2_.getScaledWidth() - i - b0 - scoreboard23.X.getValue().intValue();
        int k = 0;
        ...
        for (Object score1 : arraylist1) {
            ...
            //System.out.print(ScorePlayerTeam.formatPlayerName(scoreplayerteam1, ((Score) score1).getPlayerName()));
            //String s1 = "Hello!";
            String s2;
            if (scoreboard23.NoPotion.getValue()) {
                s2 = "";
            } else {
                s2 = EnumChatFormatting.RED + "" + ((Score) score1).getScorePoints();
            }
            int l = k1 - k * this.getFontRenderer().FONT_HEIGHT;
            int i1 = p_180475_2_.getScaledWidth() - b0 + 2 - scoreboard23.X.getValue().intValue();
            drawRect(j - 2, l, i1, l + this.getFontRenderer().FONT_HEIGHT, new Color(0,0,0,scoreboard23.RectAlpha.getValue().intValue()).getRGB());//1342177280
            if (scoreboard23.serverip.getValue()) {
                if (s1.contains("mc.remiaft.com") ||
                        s1.contains("Syuu.net") ||
                        s1.contains("ZQAT.top") ||
                        s1.contains("play.搂eHiveMC搂6.com") ||
                        s1.contains("§ewww.hypixel.ne🎂§et") ||
                        s1.contains("Play.HmXix.Top") ||
                        s1.contains("play.mcyc.win") ||
                        s1.contains("mc.mimiblock.cn") ||
                        s1.contains("www.kinomc.cn") ||
                        s1.contains("redesky.com") ||
                        s1.contains("搂emc.kinomc.搂1搂r搂ecn") ||
                        s1.contains("mc.kinomc.cn") ||
                        s1.contains("www.mcbbs.net") ||
                        s1.contains("play.cubecraft.net")
                ) {
                    if (space.emptiness.module.modules.visual.Scoreboard.serveripm.getValue() == space.emptiness.module.modules.visual.Scoreboard.serverIPMode.Normal) {
                        s1 = EnumChatFormatting.RED + "Romantic" + ".wtf";
                    } else {
                        s1 = "Romantic.wtf";
                        this.getFontRenderer().drawString(s1, j, l, ColorUtils.fastrainbow().getRGB());
                    }
                }
                if (s1.contains("mc.loyisa.cn")) {
                    if (space.emptiness.module.modules.visual.Scoreboard.serveripm.getValue() == space.emptiness.module.modules.visual.Scoreboard.serverIPMode.Normal) {
                        s1 = EnumChatFormatting.AQUA + "Loyisa " + EnumChatFormatting.BLUE + "Test";
                    } else {
                        s1 = "Loyisa Test";
                    }
            ...
            if (space.emptiness.module.modules.visual.Scoreboard.serveripm.getValue() == space.emptiness.module.modules.visual.Scoreboard.serverIPMode.Rainbow) {
                this.getFontRenderer().drawString(s1, j, l, ColorUtils.fastrainbow().getRGB());
            }
            else {
            ...
            if (k == arraylist1.size())
            {
                ...
                drawRect(j - 2, l - this.getFontRenderer().FONT_HEIGHT - 1, i1, l - 1, new Color(0,0,0,scoreboard23.RectAlpha.getValue().intValue()).getRGB());//1610612736
                drawRect(j - 2, l - 1, i1, l, new Color(0,0,0,scoreboard23.RectAlpha.getValue().intValue()).getRGB());//1342177280
