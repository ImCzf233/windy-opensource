...
public abstract class Entity implements ICommandSender
{
    ...
    /**
     * Whether this entity is currently inside of water (if it handles water move that is)
     */
     ...
     * vision if ignoreFrustumCheck or in frustum.
     */
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            EventSafeWalk safeWalkEvent = new EventSafeWalk(ff);
    ...
    /**
     * Checks if the entity is in range to vision by using the past in distance and comparing it to its average edge
    ...
    /**
     * Returns true if the entity is on fire. Used by vision to add the fire effect on rendering.
     */
    ...
    /**
     * Returns true if the entity is riding another entity, used by vision to rotate the legs to be in 'sit' position
     * for players.
     */
     ...
     * For EntityLivingBase subclasses, returning false when invisible will vision the entity semitransparent.
     */
    ...
    public final Vec2f getRotations() {
        if (this.rotation == null) {
            this.rotation = new Vec2f();
        }
        return this.rotation.setX(this.rotationYaw).setY(this.rotationPitch);
    }
    
    public final Vec2f getPrevRotations() {
        if (this.prevRotation == null) {
            this.prevRotation = new Vec2f();
        }
        return this.prevRotation.setX(this.prevRotationYaw).setY(this.prevRotationPitch);
    }
    
    public final Vec3f getMotion() {
        if (this.motion == null) {
            this.motion = new Vec3f();
        }
        return this.motion.setX(this.motionX).setY(this.motionY).setZ(this.motionZ);
    }
    ...
    public static EntityLivingBase getEniLinv(){
