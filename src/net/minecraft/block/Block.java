...
public class Block
{
    ...
    /**
     * The type of vision function called. 3 for standard block models, 2 for TESR's, 1 for liquids, -1 is no vision
     */
    ...
    public int getMixedBrightnessForBlock(IBlockAccess worldIn, BlockPos pos)
    {
        ...
        int i = worldIn.getCombinedLight(pos, Client.instance.getModuleManager().getModuleByClass(Xray.class).isEnabled() ? 100000 : block.getLightValue());
    ...
    public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
        if (Client.instance.getModuleManager().getModuleByClass(Xray.class).isEnabled() && this == Block.getBlockById(16)) {
            return true;
        }
        if (Client.instance.getModuleManager().getModuleByClass(Xray.class).isEnabled()) {
            Xray xray = (Xray)Client.instance.getModuleManager().getModuleByClass(Xray.class);
            for (Integer block : xray.getBlocks()) {
                if (this != Block.getBlockById(block)) continue;
                return true;
            }
        }
    ...
    /**
     * Used to determine ambient occlusion and culling when rebuilding chunks for vision
     */
