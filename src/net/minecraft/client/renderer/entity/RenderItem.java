...
import space.emptiness.module.modules.visual.EnchantEffect;
...
public class RenderItem implements IResourceManagerReloadListener
{
    ...
    {
        if(ModuleManager.getModuleByName("EnchantEffect").isEnabled()) {
            if (!Config.isCustomItems() || CustomItems.isUseGlint())
            {
                if (!Config.isShaders() || !Shaders.isShadowPass)
                {
                    int rendercolor;
                    rendercolor = EnchantEffect.rainbow.getValue() ? EnchantEffect.color : new Color(HUD.r.getValue().intValue(),HUD.g.getValue().intValue(),HUD.b.getValue().intValue()).getRGB();
                    ...
                    this.renderModel(model, rendercolor);
                    GlStateManager.popMatrix();
                    GlStateManager.pushMatrix();
                    GlStateManager.scale(8.0F, 8.0F, 8.0F);
